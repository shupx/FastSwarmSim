  ...
  int64_t local_steady_now_ns)
{
  ...
  switch (control.command) {
    case fastswarm_time_interfaces::msg::TimeControl::COMMAND_PAUSE:
      paused_ = true;
      break;
    case fastswarm_time_interfaces::msg::TimeControl::COMMAND_RESUME:
      paused_ = false;
      break;
    case fastswarm_time_interfaces::msg::TimeControl::COMMAND_SET_SPEED:
      set_speed(control.max_speed_ratio, local_steady_now_ns);
      break;
    case fastswarm_time_interfaces::msg::TimeControl::COMMAND_RESET:
...
GrantResult DistributedTimeCore::compute_grant(int64_t local_steady_now_ns)
{
  ...
  for (const auto & item : participants_) {
    ...
    if (participant.state == fastswarm_time_interfaces::msg::TimeIntent::STATE_ACTIVE) {
...
void DistributedTimeCore::remove_expired(int64_t local_steady_now_ns)
{
  for (auto it = participants_.begin(); it != participants_.end();) {
    ...
    const bool leaving =
      state.state == fastswarm_time_interfaces::msg::TimeIntent::STATE_LEAVING;
    if (expired || leaving) {
