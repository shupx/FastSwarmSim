  ...
  core_(0, max_speed_ratio, std::chrono::duration_cast<std::chrono::nanoseconds>(lease_timeout).count())
{
  ...
  intent_pub_ = node_.create_publisher<fastswarm_time_interfaces::msg::TimeIntent>(
    "/fastswarm/time_intent", intent_qos);
  intent_sub_ = node_.create_subscription<fastswarm_time_interfaces::msg::TimeIntent>(
    "/fastswarm/time_intent", intent_qos,
    [this](const fastswarm_time_interfaces::msg::TimeIntent::SharedPtr msg) { on_intent(msg); });

  control_sub_ = node_.create_subscription<fastswarm_time_interfaces::msg::TimeControl>(
    "/fastswarm/time_control", rclcpp::QoS(rclcpp::KeepLast(20)).reliable().transient_local(),
    [this](const fastswarm_time_interfaces::msg::TimeControl::SharedPtr msg) { on_control(msg); });
...
void TimeParticipant::start()
{
  ...
  intent_state_ = fastswarm_time_interfaces::msg::TimeIntent::STATE_IDLE;
...
void TimeParticipant::announce_next_safe_time(const rclcpp::Time & next_safe_time)
{
  ...
  intent_state_ = fastswarm_time_interfaces::msg::TimeIntent::STATE_ACTIVE;
  publish_intent(intent_state_, next_safe_time_ns_);
}
...
{
  publish_intent(fastswarm_time_interfaces::msg::TimeIntent::STATE_LEAVING, core_.current_time_ns());
}
...
void TimeParticipant::on_intent(const fastswarm_time_interfaces::msg::TimeIntent::SharedPtr msg)
{
  core_.observe_intent(*msg, steady_now_ns());
}

void TimeParticipant::on_control(const fastswarm_time_interfaces::msg::TimeControl::SharedPtr msg)
{
